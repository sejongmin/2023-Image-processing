...
int main(int argc, char* argv[]) {
	...
	/*for (int j = 1; j < height - 1; j++) {
		for (int i = 1; i < width-1; i++) {
			...
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 3; x++) {
					sum += Y1[(j + y - 1) * width + (i + x - 1)];
				}
			}
			Y1[j * width + i] = (unsigned char)(sum / 9);
		}
	}*/
	...
	for (int j = 1; j < height - 1; j++) {
		for (int i = 1; i < width - 1; i++) {
			...
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 3; x++) {
					G += Y1[(j + y - 1) * width + (i + x - 1)] * filter1[y][x];
				}
			}
	...
	for (int j = 1; j < height - 1; j++) {
		for (int i = 1; i < width - 1; i++) {
			...
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 3; x++) {
					G += Y1[(j + y - 1) * width + (i + x - 1)] * filter2[y][x];
				}
			}
	...
	for (int j = 1; j < height - 1; j++) {
		for (int i = 1; i < width - 1; i++) {
			...
			for (int y = 0; y < 3; y++) {
				for (int x = 0; x < 3; x++) {
					G += Y1[(j + y - 1) * width + (i + x - 1)] * filter3[y][x];
				}
			}
	...
	outputFile1 = fopen("AICenterY_LF1.bmp", "wb");
	...
	outputFile2 = fopen("AICenterY_LF2.bmp", "wb");
	...
	outputFile3 = fopen("AICenterY_LF3.bmp", "wb");
	...
	free(inputImg); free(outputImg1); 
	free(outputImg2); free(outputImg3);
	free(Y1); free(Y2); free(Y3); free(Y4);
	fclose(inputFile); fclose(outputFile1); 
	fclose(outputFile2); fclose(outputFile3);
