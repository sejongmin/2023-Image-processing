...
int main(int argc, char* argv[]) {
	BITMAPFILEHEADER bmpFile, bmpFile_;
	BITMAPINFOHEADER bmpInfo, bmpInfo_;
	FILE* inputFile = NULL, *inputFile_ = NULL;
	...
	inputFile_ = fopen("sju_mask.bmp", "rb");
	...
	int width_ = bmpInfo.biWidth;
	int height_ = bmpInfo.biHeight;
	int size_ = bmpInfo.biSizeImage;
	int bitCnt_ = bmpInfo.biBitCount;
	int stride_ = (((bitCnt / 8) * width) + 3) / 4 * 4;
	printf("W: %d(%d)\nH: %d\nS: %d\nD: %d\n\n", width_, stride_, height_, size_, bitCnt_);
	...
	FILE* outputFile = fopen("Output_sju_4.bmp", "wb");
